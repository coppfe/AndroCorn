...
class JNIEnv:
    def __init__(
        self, emu: "Emulator", class_loader: "JavaClassLoader", hooker: "Hooker"
    ):

        self._class_loader: "JavaClassLoader" = class_loader

        self._locals: "ReferenceTable" = ReferenceTable(start=1, max_entries=2048)
        self._globals: "ReferenceTable" = ReferenceTable(start=4096, max_entries=512000)

        self.__libc: "LibC" = LibC(emu)
    ...
    def __read_args_common(self, emu: "Emulator", args, args_type_list, arg_type) -> List:
        if arg_type == 0:
            args_items = args
            return self.__read_args(self, emu.mu, args_items, args_type_list)
        elif arg_type == 1:
            args_ptr = args
            return self.__read_args_v(self, emu.mu, args_ptr, args_type_list)
        else:
    ...
    def get_reference(self, idx) -> jobject:
        ...
        raise RuntimeError("Invalid get_reference(%#x)" % idx)

    def add_local_reference(self, obj) -> int:
    ...
    def set_local_reference(self, idx, newobj) -> None:
        if not isinstance(newobj, jobject):
            raise ValueError("Expected a jobject.")

        self._locals.set(idx, newobj)

    def get_local_reference(self, idx) -> jobject:
        r = self._locals.get(idx)
        return r

    def delete_local_reference(self, obj) -> None:
        if not isinstance(obj, jobject):
            raise ValueError("Expected a jobject.")
    ...
    def add_global_reference(self, obj) -> int:
        if not isinstance(obj, jobject):
            raise ValueError("Expected a jobject.")

        return self._globals.add(obj)

    def get_global_reference(self, idx) -> jobject:
        return self._globals.get(idx)

    def delete_global_reference(self, obj) -> None:
        if not isinstance(obj, jobject):
            raise ValueError("Expected a jobject.")
    ...
    def find_class(self, emu: "Emulator", env, name_ptr) -> int:
        ...
        pyclazz: "Class" = self._class_loader.find_class_by_name(name)
        if pyclazz is None:
        ...
        ref = self.add_local_reference(jclass(jvm_clazz))
        return ref

    @native_method
    def from_reflected_method(self, emu: "Emulator", env) -> int:
        raise NotImplementedError()

    @native_method
    def from_reflected_field(self, emu: "Emulator", env) -> int:
        raise NotImplementedError()

    @native_method
    def to_reflected_method(
        self, emu: "Emulator", env, class_idx, method_id, is_static
    ) -> int:
        """
        Converts a method ID derived from cls to a java.lang.reflect.Method or java.lang.reflect.Constructor object.
        isStatic emust be set to JNI_TRUE if the method ID refers to a static field, and JNI_FALSE otherwise.
        ...
        if method.modifier & MODIFIER_STATIC:
            emu.mu.mem_write(is_static, int(JNI_TRUE).to_bytes(4, byteorder="little"))
        else:
            emu.mu.mem_write(is_static, int(JNI_FALSE).to_bytes(4, byteorder="little"))
    ...
    def get_superclass(self, emu: "Emulator", env, clazz_idx) -> int:
        ...
        if not class_obj:
            raise RuntimeError("Not found class_obj with clazz_idx %d" % clazz_idx)
    ...
    def is_assignable_from(self, emu: "Emulator", env, clazz_idx1, clazz_idx2) -> int:
        jclazz1: "jclass" = self.get_reference(clazz_idx1)
        jclazz2: "jclass" = self.get_reference(clazz_idx2)

        # Create class instance.
        class_obj1: "Class" = jclazz1.value
        pyclass1: "JavaClassDef" = class_obj1.get_py_clazz()

        class_obj2: "Class" = jclazz2.value
        pyclass2: "JavaClassDef" = class_obj2.get_py_clazz()

        r = JNI_FALSE
        jvm_super: "JavaClassDef" = pyclass1.jvm_super
    ...
    @native_method
    def to_reflected_field(self, emu: "Emulator", env):
        raise NotImplementedError()

    @native_method
    def throw(self, emu: "Emulator", env):
        raise RuntimeError("throw is call, maybe bug")

    @native_method
    def throw_new(self, emu: "Emulator", env):
        raise RuntimeError("throw_new is call, maybe bug")

    @native_method
    def exception_occurred(self, emu: "Emulator", env):
        return 0

    @native_method
    def exception_describe(self, emu: "Emulator", env):
        raise NotImplementedError()

    @native_method
    def exception_clear(self, emu: "Emulator", env):
        """
    ...
    @native_method
    def fatal_error(self, emu: "Emulator", env):
        raise NotImplementedError()
    ...
    def new_global_ref(self, emu: "Emulator", env, jobj):
        ...
        global or local reference. Global references emust be explicitly disposed of by calling DeleteGlobalRef().
        """
    ...
    @native_method
    def alloc_object(self, emu: "Emulator", env):
        raise NotImplementedError()

    def __new_object(self, emu: "Emulator", env, clazz_idx, method_id, args, args_type):
        # Get class reference.
        jclazz = self.get_reference(clazz_idx)
        if not isinstance(jclazz, jclass):
            raise ValueError("Expected a jclass.")
        ...
        constructor_args = self.__read_args_common(
            emu, args, method.args_list, args_type
        )
    ...
    def new_object(self, emu: "Emulator", env, clazz_idx, method_id, arg1, arg2, arg3, arg4):
        return self.__new_object(
            emu, env, clazz_idx, method_id, (arg1, arg2, arg3, arg4), 0
        )

    @native_method
    def new_object_v(self, emu: "Emulator", env, clazz_idx, method_id, args_v):
        return self.__new_object(emu, env, clazz_idx, method_id, args_v, 1)

    @native_method
    def new_object_a(self, emu: "Emulator", env):
        raise NotImplementedError()
    ...
    def get_object_class(self, emu: "Emulator", env, obj_idx):
        ...
        pyclazz: 'JavaClassDef' = pyobj.__class__
    ...
    def __call_xxx_method(
        self, emu: "Emulator", env, obj_idx, method_id, args, args_type, is_wide=False
    ):
        obj = self.get_reference(obj_idx)
        ...
        name = method.name
        # To support polymorphism, the method found by method_id might be a base class method and cannot be called directly.
        # It is necessary to obtain the signature and name, and then find the actual implementation method through the subclass's find_method.

        real_method = clazz.find_method(name, sig)
    ...
    def call_object_method(
        self, emu: "Emulator", env, obj_idx, method_id, arg1, arg2, arg3, arg4
    ):
        return self.__call_xxx_method(
            emu, env, obj_idx, method_id, (arg1, arg2, arg3, arg4), 0
        )

    @native_method
    def call_object_method_v(self, emu: "Emulator", env, obj_idx, method_id, args):
        return self.__call_xxx_method(emu, env, obj_idx, method_id, args, 1)

    @native_method
    def call_object_method_a(self, emu: "Emulator", env):
        raise NotImplementedError()
    ...
    @native_method
    def call_boolean_method_a(self, emu: "Emulator", env):
        raise NotImplementedError()

    @native_method
    def call_byte_method(self, emu: "Emulator", env, obj_idx, method_id, arg1, arg2, arg3, arg4):
        return self.__call_xxx_method(
            emu, env, obj_idx, method_id, (arg1, arg2, arg3, arg4), 0
        )
    ...
    @native_method
    def call_byte_method_a(self, emu: "Emulator", env):
        raise NotImplementedError()

    @native_method
    def call_char_method(self, emu: "Emulator", env, obj_idx, method_id, arg1, arg2, arg3, arg4):
        return self.__call_xxx_method(
            emu, env, obj_idx, method_id, (arg1, arg2, arg3, arg4), 0
        )
    ...
    @native_method
    def call_char_method_a(self, emu, env):
        raise NotImplementedError()

    @native_method
    def call_short_method(self, emu: "Emulator", env, obj_idx, method_id, arg1, arg2, arg3, arg4):
        return self.__call_xxx_method(
            emu, env, obj_idx, method_id, (arg1, arg2, arg3, arg4), 0
        )
    ...
    @native_method
    def call_short_method_a(self, emu: "Emulator", env):
        raise NotImplementedError()

    # The upper layer doesn't know the number, so it's temporarily reading four registers; that shouldn't be wrong.
    @native_method
    def call_int_method(self, emu: "Emulator", env, obj_idx, method_id, arg1, arg2, arg3, arg4):
        return self.__call_xxx_method(
            emu, env, obj_idx, method_id, (arg1, arg2, arg3, arg4), 0
        )
    ...
    @native_method
    def call_int_method_a(self, emu: "Emulator", env):
        raise NotImplementedError()

    @native_method
    def call_long_method(self, emu: "Emulator", env, obj_idx, method_id, arg1, arg2, arg3, arg4):
        return self.__call_xxx_method(
            emu, env, obj_idx, method_id, (arg1, arg2, arg3, arg4), 0, True
        )

    @native_method
    def call_long_method_v(self, emu: "Emulator", env, obj_idx, method_id, args):
        return self.__call_xxx_method(emu, env, obj_idx, method_id, args, 1, True)

    @native_method
    def call_long_method_a(self, emu: "Emulator", env):
        raise NotImplementedError()

    @native_method
    def call_float_method(self, emu: "Emulator", env, obj_idx, method_id, arg1, arg2, arg3, arg4):
        return self.__call_xxx_method(
            emu, env, obj_idx, method_id, (arg1, arg2, arg3, arg4), 0
        )
    ...
    @native_method
    def call_float_method_a(self, emu: "Emulator", env):
        raise NotImplementedError()

    @native_method
    def call_double_method(self, emu: "Emulator", env, obj_idx, method_id, arg1, arg2, arg3, arg4):
        raise NotImplementedError()
        # return self.__call_xxx_method(emu: "Emulator", env, obj_idx, method_id, (arg1, arg2, arg3, arg4), 0)

    @native_method
    def call_double_method_v(self, emu: "Emulator", env, obj_idx, method_id, args):
        raise NotImplementedError()
        # return self.__call_xxx_method(emu: "Emulator", env, obj_idx, method_id, args, 1)

    @native_method
    def call_double_method_a(self, emu: "Emulator", env):
        raise NotImplementedError()

    @native_method
    def call_void_method(self, emu: "Emulator", env, obj_idx, method_id, arg1, arg2, arg3, arg4):
        self.__call_xxx_method(emu, env, obj_idx, method_id, (arg1, arg2, arg3, arg4), 0)

    @native_method
    def call_void_method_a(self, emu: "Emulator", env):
        raise NotImplementedError()

    @native_method
    def call_void_method_v(self, emu: "Emulator", env, obj_idx, method_id, args):
        self.__call_xxx_method(emu, env, obj_idx, method_id, args, 1)

    @native_method
    def call_nonvirtual_object_method(self, emu: "Emulator", env):
        raise NotImplementedError()

    @native_method
    def call_nonvirtual_object_method_v(self, emu: "Emulator", env):
        raise NotImplementedError()

    @native_method
    def call_nonvirtual_object_method_a(self, emu: "Emulator", env):
        raise NotImplementedError()

    @native_method
    def call_nonvirtual_boolean_method(self, emu: "Emulator", env):
        raise NotImplementedError()

    @native_method
    def call_nonvirtual_boolean_method_v(self, emu: "Emulator", env):
        raise NotImplementedError()

    @native_method
    def call_nonvirtual_boolean_method_a(self, emu: "Emulator", env):
        raise NotImplementedError()

    @native_method
    def call_nonvirtual_byte_method(self, emu: "Emulator", env):
        raise NotImplementedError()

    @native_method
    def call_nonvirtual_byte_method_v(self, emu: "Emulator", env):
        raise NotImplementedError()

    @native_method
    def call_nonvirtual_byte_method_a(self, emu: "Emulator", env):
        raise NotImplementedError()

    @native_method
    def call_nonvirtual_char_method(self, emu: "Emulator", env):
        raise NotImplementedError()

    @native_method
    def call_nonvirtual_char_method_v(self, emu: "Emulator", env):
        raise NotImplementedError()

    @native_method
    def call_nonvirtual_char_method_a(self, emu: "Emulator", env):
        raise NotImplementedError()

    @native_method
    def call_nonvirtual_short_method(self, emu: "Emulator", env):
        raise NotImplementedError()

    @native_method
    def call_nonvirtual_short_method_v(self, emu: "Emulator", env):
        raise NotImplementedError()

    @native_method
    def call_nonvirtual_short_method_a(self, emu: "Emulator", env):
        raise NotImplementedError()

    @native_method
    def call_nonvirtual_int_method(self, emu: "Emulator", env):
        raise NotImplementedError()

    @native_method
    def call_nonvirtual_int_method_v(self, emu: "Emulator", env):
        raise NotImplementedError()

    @native_method
    def call_nonvirtual_int_method_a(self, emu: "Emulator", env):
        raise NotImplementedError()

    @native_method
    def call_nonvirtual_long_method(self, emu: "Emulator", env):
        raise NotImplementedError()

    @native_method
    def call_nonvirtual_long_method_v(self, emu: "Emulator", env):
        raise NotImplementedError()

    @native_method
    def call_nonvirtual_long_method_a(self, emu: "Emulator", env):
        raise NotImplementedError()

    @native_method
    def call_nonvirtual_float_method(self, emu: "Emulator", env):
        raise NotImplementedError()

    @native_method
    def call_nonvirtual_float_method_v(self, emu: "Emulator", env):
        raise NotImplementedError()

    @native_method
    def call_nonvirtual_float_method_a(self, emu: "Emulator", env):
        raise NotImplementedError()

    @native_method
    def call_nonvirtual_double_method(self, emu: "Emulator", env):
        raise NotImplementedError()

    @native_method
    def call_nonvirtual_double_method_v(self, emu: "Emulator", env):
        raise NotImplementedError()

    @native_method
    def call_nonvirtual_double_method_a(self, emu: "Emulator", env):
        raise NotImplementedError()

    @native_method
    def call_nonvirtual_void_method(self, emu: "Emulator", env):
        raise NotImplementedError()

    @native_method
    def call_nonvirtual_void_method_v(self, emu: "Emulator", env):
        raise NotImplementedError()

    @native_method
    def call_nonvirtual_void_method_a(self, emu: "Emulator", env):
        raise NotImplementedError()
    ...
    def __get_xxx_field(self, emu: "Emulator", env, obj_idx, field_id, is_wide=False):
        obj = self.get_reference(obj_idx)

        if not isinstance(obj, jobject):
            raise ValueError("Expected a jobject.")

        pyobj: 'Class' = JNIEnv.jobject_to_pyobject(obj)
    ...
    @native_method
    def get_object_field(self, emu: "Emulator", env, obj_idx, field_id):
        return self.__get_xxx_field(emu, env, obj_idx, field_id)

    @native_method
    def get_boolean_field(self, emu: "Emulator", env, obj_idx, field_id):
        return self.__get_xxx_field(emu, env, obj_idx, field_id)

    @native_method
    def get_byte_field(self, emu: "Emulator", env, obj_idx, field_id):
        return self.__get_xxx_field(emu, env, obj_idx, field_id)

    @native_method
    def get_char_field(self, emu: "Emulator", env, obj_idx, field_id):
        return self.__get_xxx_field(emu, env, obj_idx, field_id)

    @native_method
    def get_short_field(self, emu: "Emulator", env, obj_idx, field_id):
        return self.__get_xxx_field(emu, env, obj_idx, field_id)

    @native_method
    def get_int_field(self, emu: "Emulator", env, obj_idx, field_id):
        return self.__get_xxx_field(emu, env, obj_idx, field_id)

    @native_method
    def get_long_field(self, emu: "Emulator", env, obj_idx, field_id):
        return self.__get_xxx_field(emu, env, obj_idx, field_id, True)

    @native_method
    def get_float_field(self, emu: "Emulator", env, obj_idx, field_id):
        return self.__get_xxx_field(emu, env, obj_idx, field_id)

    @native_method
    def get_double_field(self, emu: "Emulator", env, obj_idx, field_id):
        raise NotImplementedError()

    def __set_xxx_field(self, emu: "Emulator", env, obj_idx, field_id, value, is_obj_value=False):
        obj = self.get_reference(obj_idx)
    ...
    @native_method
    def set_object_field(self, emu: "Emulator", env, obj_idx, field_id, value):
        self.__set_xxx_field(emu, env, obj_idx, field_id, value, True)

    @native_method
    def set_boolean_field(self, emu: "Emulator", env, obj_idx, field_id, value):
        self.__set_xxx_field(emu, env, obj_idx, field_id, value)

    @native_method
    def set_byte_field(self, emu: "Emulator", env, obj_idx, field_id, value):
        self.__set_xxx_field(emu, env, obj_idx, field_id, value)

    @native_method
    def set_char_field(self, emu: "Emulator", env, obj_idx, field_id, value):
        self.__set_xxx_field(emu, env, obj_idx, field_id, value)

    @native_method
    def set_short_field(self, emu: "Emulator", env, obj_idx, field_id, value):
        self.__set_xxx_field(emu, env, obj_idx, field_id, value)

    @native_method
    def set_int_field(self, emu: "Emulator", env, obj_idx, field_id, value):
        self.__set_xxx_field(emu, env, obj_idx, field_id, value)

    @native_method
    def set_long_field(self, emu: "Emulator", env, obj_idx, field_id, value):
        self.__set_xxx_field(emu, env, obj_idx, field_id, value)

    @native_method
    def set_float_field(self, emu: "Emulator", env, obj_idx, field_id, value):
        self.__set_xxx_field(emu, env, obj_idx, field_id, value)

    @native_method
    def set_double_field(self, emu: "Emulator", env, obj_idx, field_id, value):
        self.__set_xxx_field(emu, env, obj_idx, field_id, value)
    ...
    def __call_static_xxx_method(
        self, emu: "Emulator", env, clazz_idx, method_id, args, args_type, is_wide=False
    ):
        clazz = self.get_reference(clazz_idx)
        ...
        if not is_wide:
            ...
            rhigh = raw_bits >> 32
            rlow = raw_bits & 0xFFFFFFFF
            return (rlow, rhigh)

    @native_method
    def call_static_object_method(
        self, emu: "Emulator", env, clazz_idx, method_id, arg1, arg2, arg3, arg4
    ):
    ...
    def call_static_object_method_v(self, emu: "Emulator", env, clazz_idx, method_id, args):
        return self.__call_static_xxx_method(emu, env, clazz_idx, method_id, args, 1)

    @native_method
    def call_static_object_method_a(self, emu: "Emulator", env):
        raise NotImplementedError()
    ...
    @native_method
    def call_static_boolean_method_a(self, emu: "Emulator", env):
        raise NotImplementedError()
    ...
    @native_method
    def call_static_byte_method_a(self, emu: "Emulator", env):
        raise NotImplementedError()
    ...
    @native_method
    def call_static_char_method_a(self, emu: "Emulator", env):
        raise NotImplementedError()
    ...
    @native_method
    def call_static_short_method_a(self, emu: "Emulator", env):
        raise NotImplementedError()
    ...
    @native_method
    def call_static_int_method_a(self, emu: "Emulator", env):
        raise NotImplementedError()
    ...
    def call_static_long_method(
        ...
        # raise NotImplementedError()
        return self.__call_static_xxx_method(
            emu, env, clazz_idx, method_id, (arg1, arg2, arg3, arg4), 0, True
        )
    ...
    def call_static_long_method_v(self, emu: "Emulator", env, clazz_idx, method_id, args):
        # raise NotImplementedError()
        return self.__call_static_xxx_method(
            emu, env, clazz_idx, method_id, args, 1, True
        )

    @native_method
    def call_static_long_method_a(self, emu: "Emulator", env):
        raise NotImplementedError()

    @native_method
    def call_static_float_method(
        self, emu, env, clazz_idx, method_id, arg1, arg2, arg3, arg4
    ):
        return self.__call_static_xxx_method(
            emu, env, clazz_idx, method_id, (arg1, arg2, arg3, arg4), 0
        )
    ...
    @native_method
    def call_static_float_method_a(self, emu: "Emulator", env):
        raise NotImplementedError()
    ...
    def call_static_double_method(
        ...
        raise NotImplementedError()
        # return self.__call_static_xxx_method(emu: "Emulator", env, clazz_idx, method_id, (arg1, arg2, arg3, arg4), 0)

    @native_method
    def call_static_double_method_v(self, emu: "Emulator", env, clazz_idx, method_id, args):
        raise NotImplementedError()
        # return self.__call_static_xxx_method(emu: "Emulator", env, clazz_idx, method_id, args, 1)

    @native_method
    def call_static_double_method_a(self, emu: "Emulator", env):
        raise NotImplementedError()
    ...
    def call_static_void_method_v(self, emu: "Emulator", env, clazz_idx, method_id, args):
        self.__call_static_xxx_method(emu, env, clazz_idx, method_id, args, 1)

    @native_method
    def call_static_void_method_a(self, emu: "Emulator", env):
        raise NotImplementedError()
    ...
    def __get_static_xxx_field(self, emu: "Emulator", env, clazz_idx, field_id, is_wide=False):
    ...
    @native_method
    def get_static_object_field(self, emu: "Emulator", env, clazz_idx, field_id):
        return self.__get_static_xxx_field(emu, env, clazz_idx, field_id)

    @native_method
    def get_static_boolean_field(self, emu: "Emulator", env, clazz_idx, field_id):
        return self.__get_static_xxx_field(emu, env, clazz_idx, field_id)

    @native_method
    def get_static_byte_field(self, emu, env, clazz_idx, field_id):
        return self.__get_static_xxx_field(emu, env, clazz_idx, field_id)

    @native_method
    def get_static_char_field(self, emu: "Emulator", env, clazz_idx, field_id):
        return self.__get_static_xxx_field(emu, env, clazz_idx, field_id)

    @native_method
    def get_static_short_field(self, emu: "Emulator", env, clazz_idx, field_id):
        return self.__get_static_xxx_field(emu, env, clazz_idx, field_id)

    @native_method
    def get_static_int_field(self, emu: "Emulator", env, clazz_idx, field_id):
        return self.__get_static_xxx_field(emu, env, clazz_idx, field_id)

    @native_method
    def get_static_long_field(self, emu: "Emulator", env, clazz_idx, field_id):
        return self.__get_static_xxx_field(emu, env, clazz_idx, field_id, True)

    @native_method
    def get_static_float_field(self, emu: "Emulator", env, clazz_idx, field_id):
        return self.__get_static_xxx_field(emu, env, clazz_idx, field_id)

    @native_method
    def get_static_double_field(self, emu: "Emulator", env, clazz_idx, field_id):
        return self.__get_static_xxx_field(emu, env, clazz_idx, field_id, True)

    @native_method
    def set_static_object_field(self, emu: "Emulator", env):
        raise NotImplementedError()

    @native_method
    def set_static_boolean_field(self, emu: "Emulator", env):
        raise NotImplementedError()

    @native_method
    def set_static_byte_field(self, emu: "Emulator", env):
        raise NotImplementedError()

    @native_method
    def set_static_char_field(self, emu: "Emulator", env):
        raise NotImplementedError()

    @native_method
    def set_static_short_field(self, emu: "Emulator", env):
        raise NotImplementedError()

    @native_method
    def set_static_int_field(self, emu: "Emulator", env):
        raise NotImplementedError()

    @native_method
    def set_static_long_field(self, emu: "Emulator", env, clazz_idx, field_id, _, value_l, value_h):
        # NOTE: that since the fourth parameter is exactly 8 bytes.
        # The arm32 will not use R3 as a register to pass the parameter, but will skip R3 and use the stack directly.
        ...
        field = pyclazz.find_field_by_id(field_id)
        # FIXME: Field support is not yet complete; non-stativ values ​​cannot be set and need improvement.
        field.static_value = value

    @native_method
    def set_static_float_field(self, emu: "Emulator", env):
        raise NotImplementedError()

    @native_method
    def set_static_double_field(self, emu: "Emulator", env):
        raise NotImplementedError()

    @native_method
    def new_string(self, emu: "Emulator", env):
        raise NotImplementedError()

    @native_method
    def get_string_length(self, emu: "Emulator", env):
        raise NotImplementedError()

    @native_method
    def get_string_chars(self, emu: "Emulator", env):
        raise NotImplementedError()

    @native_method
    def release_string_chars(self, emu: "Emulator", env):
        raise NotImplementedError()
    ...
    def get_string_utf_chars(self, emu: "Emulator", env, string, is_copy_ptr):
        ...
        str_val: str = str_ref.value.get_py_string()
        utf8_bytes = str_val.encode("utf-8") + b"\x00"
    ...
    def release_string_utf_chars(self, emu: "Emulator", env, string, utf8_ptr):
        # pystr = memory_helpers.read_utf8(emu: "Emulator", utf8_ptr)

        if utf8_ptr != 0:
            self.__libc.free(utf8_ptr)
            # self._eemu.memory.unmap(utf8_ptr, len(pystr)+1)
    ...
    def new_object_array(self, emu: "Emulator", env, size, class_idx, obj_init):
        ...
        if pyarray_clazz == None:
            ...
            # Dynamically create a new Array class because the Descriptor will change.
    ...
    @native_method
    def new_boolean_array(self, emu: "Emulator", env):
        raise NotImplementedError()
    ...
    @native_method
    def new_char_array(self, emu: "Emulator", env):
        raise NotImplementedError()

    @native_method
    def new_short_array(self, emu: "Emulator", env):
        raise NotImplementedError()

    @native_method
    def new_int_array(self, emu: "Emulator", env):
        raise NotImplementedError()

    @native_method
    def new_long_array(self, emu: "Emulator", env):
        raise NotImplementedError()

    @native_method
    def new_float_array(self, emu: "Emulator", env):
        raise NotImplementedError()

    @native_method
    def new_double_array(self, emu: "Emulator", env):
        raise NotImplementedError()

    @native_method
    def get_boolean_array_elements(self, emu: "Emulator", env):
        raise NotImplementedError()
    ...
    def get_byte_array_elements(self, emu: "Emulator", env, array_idx, is_copy_ptr):
        ...
        pyobj: "Array" = JNIEnv.jobject_to_pyobject(obj)
        ...
        b = bytes(items)
        emu.mu.mem_write(buf + extra_n, b)
        return buf + extra_n

    @native_method
    def get_char_array_elements(self, emu: "Emulator", env):
        raise NotImplementedError()

    @native_method
    def get_short_array_elements(self, emu: "Emulator", env):
        raise NotImplementedError()

    @native_method
    def get_int_array_elements(self, emu: "Emulator", env):
        raise NotImplementedError()

    @native_method
    def get_long_array_elements(self, emu: "Emulator", env):
        raise NotImplementedError()

    @native_method
    def get_float_array_elements(self, emu: "Emulator", env):
        raise NotImplementedError()

    @native_method
    def get_double_array_elements(self, emu: "Emulator", env):
        raise NotImplementedError()

    @native_method
    def release_boolean_array_elements(self, emu: "Emulator", env):
        raise NotImplementedError()
    ...
    def release_byte_array_elements(self, emu: "Emulator", env, array_idx, elems, mode):
        ...
        true_buf = elems - 8
        self.__libc.free(true_buf)

        # b = emu.mem_read(true_buf, 4)
        # elems_sz =  int.from_bytes(b, byteorder='little', signed = False)
        # self._eemu.memory.unmap(true_buf, elems_sz+4)

    @native_method
    def release_char_array_elements(self, emu: "Emulator", env):
        raise NotImplementedError()

    @native_method
    def release_short_array_elements(self, emu: "Emulator", env):
        raise NotImplementedError()

    @native_method
    def release_int_array_elements(self, emu: "Emulator", env):
        raise NotImplementedError()

    @native_method
    def release_long_array_elements(self, emu: "Emulator", env):
        raise NotImplementedError()

    @native_method
    def release_float_array_elements(self, emu: "Emulator", env):
        raise NotImplementedError()

    @native_method
    def release_double_array_elements(self, emu: "Emulator", env):
        raise NotImplementedError()

    @native_method
    def get_boolean_array_region(self, emu: "Emulator", env):
        raise NotImplementedError()
    ...
    @native_method
    def get_char_array_region(self, emu: "Emulator", env):
        raise NotImplementedError()

    @native_method
    def get_short_array_region(self, emu: "Emulator", env):
        raise NotImplementedError()

    @native_method
    def get_int_array_region(self, emu: "Emulator", env):
        raise NotImplementedError()

    @native_method
    def get_long_array_region(self, emu: "Emulator", env):
        raise NotImplementedError()

    @native_method
    def get_float_array_region(self, emu: "Emulator", env):
        raise NotImplementedError()

    @native_method
    def get_double_array_region(self, emu: "Emulator", env):
        raise NotImplementedError()

    @native_method
    def set_boolean_array_region(self, emu: "Emulator", env):
        raise NotImplementedError()
    ...
    @native_method
    def set_char_array_region(self, emu: "Emulator", env):
        raise NotImplementedError()

    @native_method
    def set_short_array_region(self, emu: "Emulator", env):
        raise NotImplementedError()

    @native_method
    def set_int_array_region(self, emu: "Emulator", env):
        raise NotImplementedError()

    @native_method
    def set_long_array_region(self, emu: "Emulator", env):
        raise NotImplementedError()

    @native_method
    def set_float_array_region(self, emu: "Emulator", env):
        raise NotImplementedError()

    @native_method
    def set_double_array_region(self, emu: "Emulator", env):
        raise NotImplementedError()
    ...
    def register_natives(self, emu: "Emulator", env, clazz_id, methods, methods_count):
        ...
        for i in range(0, methods_count):
            ptr_name = helpers.read_ptr_sz(emu.mu, (i * 3 * ptr_sz) + methods)
            ptr_sign = helpers.read_ptr_sz(
                emu.mu, (i * 3 * ptr_sz) + methods + ptr_sz
            )
            ptr_func = helpers.read_ptr_sz(
                emu.mu, (i * 3 * ptr_sz) + methods + 2 * ptr_sz
            )
    ...
    @native_method
    def unregister_natives(self, emu: "Emulator", env):
        raise NotImplementedError()

    @native_method
    def monitor_enter(self, emu: "Emulator", env):
        raise NotImplementedError()

    @native_method
    def monitor_exit(self, emu: "Emulator", env):
        raise NotImplementedError()

    @native_method
    def get_java_vm(self, emu: "Emulator", env, vm):

        emu.mu.mem_write(vm, emu.java_vm.address_ptr.to_bytes(4, byteorder="little"))

        return JNI_OK

    @native_method
    def get_string_region(self, emu: "Emulator", env):
        raise NotImplementedError()

    @native_method
    def get_string_utf_region(self, emu: "Emulator", env):
        raise NotImplementedError()

    @native_method
    def get_primitive_array_critical(self, emu: "Emulator", env):
        raise NotImplementedError()

    @native_method
    def release_primitive_array_critical(self, emu: "Emulator", env):
        raise NotImplementedError()

    @native_method
    def get_string_critical(self, emu: "Emulator", env):
        raise NotImplementedError()

    @native_method
    def release_string_critical(self, emu: "Emulator", env):
        raise NotImplementedError()

    @native_method
    def new_weak_global_ref(self, emu: "Emulator", env):
        raise NotImplementedError()

    @native_method
    def delete_weak_global_ref(self, emu: "Emulator", env):
        raise NotImplementedError()

    @native_method
    def exception_check(self, emu: "Emulator", env):
        """
    ...
    @native_method
    def new_direct_byte_buffer(self, emu: "Emulator", env):
        raise NotImplementedError()

    @native_method
    def get_direct_buffer_address(self, emu: "Emulator", env):
        raise NotImplementedError()

    @native_method
    def get_direct_buffer_capacity(self, emu: "Emulator", env):
        raise NotImplementedError()
