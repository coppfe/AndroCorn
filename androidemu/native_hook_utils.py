...
class FuncHooker:
    ...
    def __hook_func_head(self, mu: 'Uc', address, size, user_data):
        try:
            ...
            # if someone can make it readable i wait for pull request
    ...
    def fun_hook(self, fun_addr: int, nargs: int, cb_before: Callable, cb_after: Optional[Callable] = None):
        # TODO: optimize. maybe... too many fun hooks = slower

        fun_addr = standlize_addr(fun_addr)
        mu = self.__emu.mu
        mu.hook_add(UC_HOOK_CODE, self.__hook_func_head, None, fun_addr, fun_addr)
