...
class ProcessHelper:
    ...
    def _do_fork(self, sch: 'Scheduler'):
    ...
    def _clone(self, emu: 'Emulator', flags, child_stack, parent_tid, new_tls, child_tid):
        mu = emu.mu
        ...
        if (flags & FORK_FLAGS == FORK_FLAGS or 
            ...
            return self._do_fork(emu.scheduler)
    
        elif(flags & THREAD_FLAGS == THREAD_FLAGS):
            tls_ptr = 0
            if (flags & (THREAD_TLS_INIT_FLAGS) != 0):
                tls_ptr = new_tls
            tid = emu.scheduler.add_sub_task(child_stack, tls_ptr)
            ...
            if (flags & CLONE_CHILD_SETTID) and child_tid != 0:
