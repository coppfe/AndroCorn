...
class ExecveHandler:
    def __init__(self, pcb: 'ProcessControlBlock', system_properties: Dict[str, str], config: 'Config', ctx: 'GlobalContextMachine'):
        self._pcb = pcb
    ...
    def write_stdout(self, msg: str):
        pipe_out = self._pcb.virtual_files.get_fd_detail(1)
    ...
    def execute(self, filename: str, argv: List[str]) -> int:
        ...
        if self._pcb.virtual_files.has_fd(1):
            self._pcb.virtual_files.remove_fd(1)
    ...
    def _handle_id(self, argv: List[str]) -> int:
        logging.debug("[EXECVE MOCK] id -> uid=%d", self._ctx.uid)
        return 0
    ...
    def _logic_pm_path(self, pkg: str) -> int:
        if pkg == self._config.pkg.pkg_name:
